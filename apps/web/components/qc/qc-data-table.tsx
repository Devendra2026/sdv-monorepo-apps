...
import { cn, fmtDay } from "@workspace/ui/lib/utils"
...
const QcDataTableDesktopRow = memo(function QcDataTableDesktopRow({
  row,
  index,
  pageStart,
  hrefBase,
  ...
  return (
    <TableRow className={cn(QC_TABLE.bodyRow, surveyRowTone(row))}>
      <TableCell className={cn(QC_TABLE.bodyCell, "text-muted-foreground tabular-nums")}>
        {pageStart + index + 1}
      </TableCell>
      ...
      <TableCell className={cn(QC_TABLE.bodyCell, "font-mono text-xs tabular-nums")}>
...
const QcDataTableMobileCard = memo(function QcDataTableMobileCard({
  row,
  index,
  pageStart,
  hrefBase,
  ulbCodes,
  showSurveyor,
}: Pick<RowRenderProps, "row" | "index" | "pageStart" | "hrefBase" | "ulbCodes" | "showSurveyor">) {
  return (
    <article className={cn("rounded-xl border border-border/70 bg-card p-4 shadow-sm", surveyRowTone(row))}>
...
function QcDataTableDesktop({
  ...
  const paddingBottom =
    virtualRows && virtualRows.length > 0 ? virtualizer.getTotalSize() - virtualRows[virtualRows.length - 1]!.end : 0

  return (
    <div className={cn(QC_TABLE.wrapper, "hidden lg:block")}>
      <div ref={scrollRef} className={cn(QC_TABLE.scroll, shouldVirtualize && "max-h-[min(70vh,720px)]")}>
        <Table className={QC_TABLE.table}>
          ...
          <TableBody>
            {shouldVirtualize ? (
              <>
                {paddingTop > 0 && (
                  <TableRow aria-hidden>
                    <TableCell colSpan={showSurveyor ? 11 : 10} style={{ height: paddingTop, padding: 0, border: 0 }} />
                  </TableRow>
                )}
                {virtualRows!.map((virtualRow) => {
                  const row = rows[virtualRow.index]!
                  return <QcDataTableDesktopRow key={row._id} row={row} index={virtualRow.index} {...rowProps} />
                })}
                {paddingBottom > 0 && (
                  <TableRow aria-hidden>
                    <TableCell
                      colSpan={showSurveyor ? 11 : 10}
              ...
              rows.map((row, index) => <QcDataTableDesktopRow key={row._id} row={row} index={index} {...rowProps} />)
            )}
          </TableBody>
        </Table>
      </div>
  ...
  return (
    <>
      <QcDataTableDesktop
        ...
        showDemandActions={showDemandActions}
      />

      <div className="grid gap-3 lg:hidden">
        {rows.map((row, index) => (
          <QcDataTableMobileCard
            key={row._id}
            row={row}
            index={index}
            pageStart={pageStart}
