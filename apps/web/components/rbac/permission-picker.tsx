...
export function PermissionPicker({
  ...
  const grouped = useMemo(() => {
    ...
    const filtered = q
      ? permissions.filter(
          (p) =>
            p.key.toLowerCase().includes(q) || p.label.toLowerCase().includes(q) || p.category.toLowerCase().includes(q)
        )
      : permissions
                        ...
                        <li key={p.key}>
                          ...
                          >
                            ...
                            <span className="min-w-0 flex-1">
                              <span className="block leading-snug font-medium">{p.label}</span>
