...
type RoleList = FunctionReturnType<typeof api.rbac.queries.listRoles>
type PermissionList = FunctionReturnType<typeof api.rbac.queries.listPermissions>

type RolesClientProps = {
  preloadedRoles: Preloaded<typeof api.rbac.queries.listRoles>
  preloadedPermissions: Preloaded<typeof api.rbac.queries.listPermissions>
}

export function RolesClient({ preloadedRoles, preloadedPermissions }: RolesClientProps) {
  const seedRoles = usePreloadedQuery(preloadedRoles)
  const seedPermissions = usePreloadedQuery(preloadedPermissions)
  const liveRoles = useRoles({ includeInactive: true })
  const livePermissions = usePermissions()
  const roles: RoleList | undefined = liveRoles ?? seedRoles
  const permissions: PermissionList | undefined = livePermissions ?? seedPermissions
    ...
    >
      <div className="space-y-6 lg:space-y-8">
        ...
        <p className="text-center text-xs text-muted-foreground">
          Permission changes apply reactively when Convex refreshes the signed-in user's session data.
        </p>
      </div>
    </RoleGate>
  )
