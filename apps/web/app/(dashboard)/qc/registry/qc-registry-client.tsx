...
function QcRegistryContent({ seedStats }: { seedStats?: CommandCenterStats }) {
  ...
  const {
    ...
    goNext,
    goPrev,
  } = useQcRegistry({ initialTab: tabFromUrl ?? "active", seedStats })
...
export function QcRegistryClient({
  preloadedStats,
}: {
  ...
  return (
    ...
    >
      <Suspense fallback={<QcPageSkeleton variant="registry" />}>
        <QcRegistryContent seedStats={seedStats} />
      </Suspense>
    </RoleGate>
  )
}
