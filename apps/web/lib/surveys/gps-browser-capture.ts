import { surveyLibErr, surveyLibOk, type SurveyLibResult } from "@/lib/survey/result"
...
/** Safe wrapper returning a typed result instead of throwing. */
export async function captureBrowserGpsSafe(): Promise<SurveyLibResult<BrowserGpsFix>> {
  try {
    const data = await captureBrowserGps()
    return surveyLibOk(data)
  } catch (error) {
    const message = error instanceof Error ? error.message : "Failed to capture GPS location."
    const code =
      message.includes("permission") || message.includes("denied")
        ...
        : message.includes("timed out")
          ? "GPS_TIMEOUT"
          : "GPS_CAPTURE_FAILED"
    return surveyLibErr(code, message)
  }
}
