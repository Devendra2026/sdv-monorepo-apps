...
/** Non-admin callers may only read users in their tenant scope (profile or allotments). */
export function isUserVisibleInScope(
  me: Doc<"users">,
  target: Doc<"users">,
  ...
  if (target.municipalityId && muniIds.has(target.municipalityId)) return true
  if (target.districtId && districtIds.has(target.districtId)) return true
  return false
}

export async function assertUserVisibleToCaller(ctx: QueryCtx, me: Doc<"users">, target: Doc<"users">): Promise<void> {
  if (me.role === "admin") return
  const scope = await resolveTenantScope(ctx, me)
  if (isUserVisibleInScope(me, target, tenantMunicipalityIds(scope), tenantDistrictIds(scope))) return
  clientError("FORBIDDEN", "User is outside your assigned scope")
}

export async function filterUsersToCallerScope(
  ctx: QueryCtx,
  me: Doc<"users">,
  users: Doc<"users">[]
): Promise<Doc<"users">[]> {
  ...
  return users.filter((u) => isUserVisibleInScope(me, u, muniIds, districtIds))
}
